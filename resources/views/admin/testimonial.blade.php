        ...
        <div class="inner" style="min-height:1200px;">
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <h2>Feedbacks/Testimonial Management</h2>
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12">
                    ...
                    <table class="table table-hover table-responsive">
                        ...
                        <tr>
                            <td>{{$item->name}}</td>
                            <td>@foreach(range(1,$item->points) as $x) <i class="fa fa-star"></i> @endforeach</td>
                            <td>{{substr($item->content,0,50)}}</td>
                            <td><a href="{{url('/admin/testimonial/delete/'.$item->id)}}" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></a> <a href="{{url('/admin/testimonial/edit/'.$item->id)}}" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a> <a href="{{url('/admin/testimonial/'.$item->id)}}" class="btn btn-warning btn-xs"><i class="fa fa-eye"></i></a></td>
                            <td>
                                <div class="make-switch switch-mini">
                                    <input type="checkbox" @if($item->status == 1)checked="checked" onchange="window.location.href='{{url('/admin/testimonial/hide/'.$item->id)}}'" @else onchange="window.location.href='{{url('/admin/testimonial/show/'.$item->id)}}'" @endif  />
                                </div>
                            </td>
                        </tr>
                        @endforeach
