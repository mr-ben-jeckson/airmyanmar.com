...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <div class="col-lg-4 form-group">
                        ...
                        <input type="text" class="form-control" name="bid" placeholder="Booking ID" @if($request->bid) value="{{$request->bid}}" @endif>
                    </div>
                    <div class="col-lg-4 form-group">
                        <label>Find by Booking Date</label>
                        <input type="text" class="form-control" name="booking_date" placeholder="MM/YY/DD" id="dp2" data-date-format="mm/dd/yyyy" @if($request->booking_date) value="{{$request->booking_date}}" @endif>
                    </div>
                    <div class="col-lg-4 form-group">
                        <label>Find by Travel Date</label>
                        <input type="text" class="form-control" name="dep_date" placeholder="MM/YY/DD" id="dp3" data-date-format="mm/dd/yyyy" @if($request->dep_date) value="{{$request->dep_date}}" @endif>
                    </div>
                    <div class="col-lg-4 form-group">
                        <label>Find by Customer Email</label>
                        <input type="email" class="form-control" name="c_email" placeholder="Email" @if($request->c_email) value="{{$request->c_email}}" @endif>
                    </div>
                    <div class="col-lg-4 form-group">
                        <label>Find by Airline PNR</label>
                        <input type="text" class="form-control" name="pnr" placeholder="PNR" @if($request->pnr) value="{{$request->pnr}}" @endif>
                    </div>
                    <div class="col-lg-4 form-group">
                        ...
                        <select name="status" class="form-control" tabindex="2">
                            ...
                            <option value="1" @if($request->status && $request->status == 1) selected @endif>Processing List</option>
                            <option value="2" @if($request->status && $request->status == 2) selected @endif>Awaiting Payment</option>
                            <option value="3" @if($request->status && $request->status == 3) selected @endif>Completed List</option>
                            <option value="4" @if($request->status && $request->status == 4) selected @endif>Final Completed</option>
                            <option value="5" @if($request->status && $request->status == 5) selected @endif>Cancelled as Fully Booked</option>
                            <option value="6" @if($request->status && $request->status == 6) selected @endif>Cancelled List</option>
                        </select>
                    </div>
 ...
 $departure = date('l, M d, Y',$time); $arrival = date('l, M d, Y',$time1); if ($item->route == "oneway"){ $flight = \App\tickets::find($item->ticket);}else{( $flight = \App\tickets::find(unserialize($item->ticket)['0']));}?>
                    ...
                    @endforeach
                    @else
                        <?php $time = strtotime($booking->dep_date); $time1 = strtotime($booking->return_date);
                        $departure = date('l, M d, Y',$time); $arrival = date('l, M d, Y',$time1); if ($booking->route == "oneway"){ $flight = \App\tickets::find($booking->ticket);}else{ $flight = \App\tickets::find(unserialize($booking->ticket)['0']);}?>
                        <div class="col-lg-4">
                            <div class="col-lg-12 booking-box">
                                @if($booking->status == 1)
                                    <i class="fa fa-bell" style="color: #ffe128;"></i> Processing List
                                ...
                                @endif
                                <hr>
                                <i class="fa fa-key"></i> ID : <b>{{$booking->booking_id}}</b><br>
                                <i class="fa fa-user-secret"></i>Username : <b>{{$booking->customer}}</b><br>
                                <i class="fa fa-plane"></i> Route : <b>@if($booking->route == "oneway") One Way Flight @else Round Flights @endif</b> <br>
                                <i class="fa fa-users"></i> Passenger : <b>{{(unserialize($booking->pax)['adult'])+(unserialize($booking->pax)['child'])+(unserialize($booking->pax)['infant'])}}</b> <br>
                                <i class="fa fa-calendar-check-o"></i> Travel Date : <b>{{$departure}}</b> <br>
                                <i class="fa fa-calendar-check-o"></i> Return Date : @if($booking->route == "oneway") <b>None</b> @else <b>{{$arrival}}</b> @endif <br>
                                <i class="fa fa-map"></i> Sectors : @if($booking->route == "oneway")<b>{{$flight->from}}</b> to <b>{{$flight->to}}</b> @else <b>{{$flight->from}}</b> to <b>{{$flight->to}}</b> to <b>{{$flight->from}}</b> @endif
                                <br>
                                <i class="icon-calendar"></i> Booking Date: <b>{{$booking->booking_date}}</b>
                                <hr>
                                <a href="{{url('/admin/booking/detail/'.$booking->id)}}" class="btn btn-primary btn-block"> <i class="fa fa-eye"></i> View Detail</a>
                            </div>
                        </div>
                    @endif
                </div>
