                        ...
                        <table class="table table-hover table-responsive">
                            ...
                            @foreach($data as $item)
                                <tr>
                                    ...
                                    <td><a href="{{url('/admin/banner/delete/'.$item->id)}}" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></a> <a href="{{url('/admin/banner/edit/'.$item->id)}}" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a></td>
                                    <td>
                                        <div class="make-switch switch-mini">
                                            <input type="checkbox" @if($item->status == 1)checked="checked" onchange="window.location.href='{{url('/admin/banner/hide/'.$item->id)}}'" @else onchange="window.location.href='{{url('/admin/banner/show/'.$item->id)}}'" @endif  />
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                            <footer>
                                <tr>
                                    <td colspan="5" style="text-align: center;"><a href="#" class="text-center">Banner Advertisements</a></td>
                                </tr>
                            </footer>
                        </table>
                    </div>
